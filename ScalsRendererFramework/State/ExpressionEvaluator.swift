...
/// - Dynamic array indexing: `"items[currentIndex]"`, `"items[index]"` (where index is a state variable)
...
/// - Complex arithmetic: `"(index + 1) % 3"`, `"(a + b) * c"`, `"value + 2 - 1"`
...
/// let cycleIndex = evaluator.evaluate("(currentIndex + 1) % 3", using: stateStore)
