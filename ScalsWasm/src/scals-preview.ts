 ...
 * Usage:
 * ```typescript
 * import { initClads, renderScalsDocument } from '@clads/preview';
 *
 * await initClads();
 * const html = renderScalsDocument(jsonDocument);
 * ```
 */
 ...
 */
export async function initClads(wasmPath?: string): Promise<void> {
    if (isInitialized) {
        return;
    }

    const path = wasmPath ?? new URL('./scals-preview.wasm', import.meta.url).href;
    ...
    try {
        ...
        const { instance } = await WebAssembly.instantiate(wasmBuffer, {
            wasi_snapshot_preview1: createWasiShim(memoryRef) as WebAssembly.ModuleImports
        });
        ...
        try {
            ...
            console.error('[CLADS] Error stack:', (initError as Error).stack);
            throw initError;
        }
 ...
 */
export function renderScalsDocument(json: string | object): string {
    if (!wasmExports) {
 ...
 */
export function isCladsInitialized(): boolean {
 ...
 */
export function resetClads(): void {
    wasmInstance = null;
    wasmExports = null;
    isInitialized = false;
}
 ...
 */
function createWasiShim(memoryRef: { memory?: WebAssembly.Memory }): object {
    return {
        ...
        // Clock
        clock_res_get: (clockId: number, resPtr: number) => {
            if (memoryRef.memory) {
                const view = new DataView(memoryRef.memory.buffer);
